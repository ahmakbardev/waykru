...
@section('content')
    <div class="bg-indigo-600 px-8 pt-10 lg:pt-14 pb-16 flex justify-between items-center mb-3">
        <!-- title -->
        <h1 class="text-xl text-white">Tambah Informasi Baru</h1>
    </div>
    <div class="mx-6 my-6">
        <div class="card h-full shadow">
            <div class="border-b border-gray-300 px-5 py-4">
                <h4>Form Tambah Informasi</h4>
            </div>
            <div class="p-6">
                <form action="{{ route('information.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="content" name="content" class="w-full border border-gray-300 p-2 rounded-md"></textarea>
                        @error('content')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">Simpan</button>
                    <a href="{{ route('information.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-md">Batal</a>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            CKEDITOR.ClassicEditor.create(document.querySelector('#content'), {
                ...
                // Ensure CKEditor content is correctly handled during form submission
